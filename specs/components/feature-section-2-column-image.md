...
Component ID: `feature-section-2-column-image`
Registry: `registry/components.json`
Status: `stable`
Client boundary: `server`
Category: `feature-section`
Library target: `library/src/components/FeatureSection2ColumnImage.tsx`
Raw snapshot: `Components/Feature Section 2 Column Image/Feature Section 2 Column Image.tsx`

## Purpose

Explains a focused product capability where the visual carries part of the message. It is the simpler two-column feature pattern without bullets.

Registry description: Two-column feature section with copy and visual container.

## When To Use

- Use for a focused product capability where the image carries part of the explanation.
...
- Do not use when multiple equivalent features should be compared side by side.
...
- default: heading, body, and image are visible and balanced.
- hover: static section, no required hover state.
- focus: any links or controls need visible focus.
- empty: do not render without heading and body.
- responsive: stack cleanly with stable image dimensions.

## Accessibility Rules

- Use meaningful alt text for product or business-context images.
...
Approved screenshot: `assets/screenshots/feature-section-2-column-image.png`
...
- Use between hero and proof sections for a single capability.
- Alternate image position only when it improves page rhythm.
- Do not use where a bullet checklist is needed.
...
- Use approved peach background and primary card radius where the snapshot calls for it.
- Keep image containers stable and non-cropping where inspection matters.
- Use a real local media asset through `ImageAsset`; do not nest another component screenshot as the feature visual.
- Do not add generic SaaS gradient panels.
...
- Before using `feature-section-2-column-image`, read this spec after reading `registry/components.json` and `tokens/design-tokens.json`.
...
- Use `library/src/components/FeatureSection2ColumnImage.tsx` as the intended future import path once Phase 4 creates the clean library implementation.
- Never import from `Components/Feature Section 2 Column Image/Feature Section 2 Column Image.tsx`; it remains a raw visual/code snapshot.
...
- `body` (required): Approved body content/input slot.
- `image` (optional): Approved image content/input slot.
- `reversed` (optional): Approved reversed content/input slot.

## Markdown Call Syntax

```markdown
:::loyaltymaster-component id="feature-section-2-column-image"
props:
