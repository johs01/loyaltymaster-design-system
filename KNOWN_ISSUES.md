...
| The Wave 2 promoted components and the canon-promoted `strategy-sentence-cards` are `stable` with Phase 5 smoke evidence (Wave 2 also has Phase 13 evidence approval) but are not yet through the Phase 7C/7E/7F visual, interaction, and production-fidelity gates. Their visual-gate entries are coverage-only (`gate: false`). | Their references were re-captured and approved in the 2026-08-29 regeneration; closing still requires flipping their gates to `true` and live loyaltymaster.com gate targets; see the Gate Coverage section in `registry/README.md`. | Open — future gate-extension session |
| The Phase 7E interaction audit and Phase 7F production targets point at `sendpush.loyaltymaster.com`, retired in July 2026 (no DNS record). The frozen Wave 1 artifacts remain valid gate-time evidence, but `verify:production-fidelity` cannot re-run. | Repointing targets at live loyaltymaster.com URLs needs a human-approved gate session (same session as the row above). | Open — future gate-extension session |
| A 2026-08-28 diagnostic `verify:visual` run found `geo-fence-use-case`, `image-grid`, and `image-strip` exceed the strict Phase 7C thresholds by tiny margins (meanDelta 0.0012–0.0021 vs 0.001) under the current capture environment (Chromium 148.0.7778.96, Darwin 25.5.0); the committed references were approved under Chromium 148.0.7778.168, Darwin 25.4.0. Image resampling drift, not a library change — dimensions matched exactly and fonts loaded. | Resolved by the 2026-08-29 human-approved Phase 7C reference regeneration: all 34 canonical references were re-captured under the current environment, reviewed old-vs-new by the owner, and approved; `npm run verify:visual` passes all 20 gated components against the new references. | 2026-08-29 reference regeneration |
