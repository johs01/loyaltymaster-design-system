...
| A 2026-08-28 diagnostic `verify:visual` run found `geo-fence-use-case`, `image-grid`, and `image-strip` exceed the strict Phase 7C thresholds by tiny margins (meanDelta 0.0012–0.0021 vs 0.001) under the current capture environment (Chromium 148.0.7778.96, Darwin 25.5.0); the committed references were approved under Chromium 148.0.7778.168, Darwin 25.4.0. Image resampling drift, not a library change — dimensions match exactly and fonts load. The committed May artifacts remain the approved evidence. | Fresh captures need a human-approved Phase 7C reference regeneration (same session as the gate rows above); regenerating without approval is forbidden. | Open — future gate-extension session |
