...
- Field is flat `color.background.peach` (`#fef0e9`). Headline and wordmark
  are `color.ink` (`#302127`).
...
- Text column: x 64 / y 64, 520px wide. Headline in Rodger Bold 700 at 84px
  ...
  Wordmark is live text "Loyaltymaster" in Lato 900 at 40px with -0.035em
  tracking — the `.brand-logo` treatment. The wordmark stays ink: yellow
  means action, so no gold wordmark on a light field. No other copy on the
  card (owner decision, 2026-08-12): messengers render og:title and
  og:description directly under the image, so a tagline reads twice.
...
- Delivery: the og:image URL must answer 200 directly — link scrapers
  (WhatsApp above all) do not reliably follow redirects, and a redirected
  card URL is how a preview silently loses its image. Scrapers also cache
  the fetched image by URL, so the card URL carries a version query that is
  bumped on every redesign.
- On loyaltymaster.com the renderer is `src/lib/og.tsx`, served as
  `/og/{slug}.jpg` by `src/app/og/[slug]/route.ts` (the dotted last segment
