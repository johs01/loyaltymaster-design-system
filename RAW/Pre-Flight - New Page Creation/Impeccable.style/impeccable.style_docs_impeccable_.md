---
url: "https://impeccable.style/docs/impeccable/"
title: "impeccable | Impeccable"
---

[Skip to content](https://impeccable.style/docs/impeccable/#main)[/Impeccable](https://impeccable.style/)
...
[Docs](https://impeccable.style/docs)/impeccableCreate
...
Every command reads both files before generating. **Register** decides which defaults load. Brand (marketing, landing, portfolio, where design IS the product) and product (app UI, dashboards, tools, where design SERVES the product) have different defaults for type, motion, color, and density. Specifying it once in PRODUCT.md means `/impeccable typeset` will not push editorial-magazine fonts on a dashboard, and will not push product-fluent defaults on a campaign page. See the [brand vs product tutorial](https://impeccable.style/tutorials/brand-vs-product) for how the two diverge.
